...
def obtener_codigo_palabra(palabra, codigo_pais):
    palabras = {
        "Ansiedad": "/m/0k_9",
        "Depresión": "/m/03f_cb",
        "Desánimo": "desánimo",
    ...
    if palabra == "Desánimo":
        return desanimo_traducciones.get(codigo_pais, "Desánimo")
    else:
        return palabras.get(palabra, "Palabra no encontrada")
    ...
    for palabra in palabras.keys():
        for codigo in paises.values():
            ...
            if os.path.isfile(archivo):
                ...
                palabra_buscada=obtener_codigo_palabra(palabra, codigo)
                pytrend.build_payload(kw_list=[palabra_buscada],geo=codigo,timeframe=year+'-01-01 '+year+'-12-31')
                # Interest Over Time
